...
const WhiteboardPage = () => {
  ...
  const saveToBackend = async () => {
    const subject = sessionStorage.getItem('currentWhiteboardSubject');
    if (!subject) return;
    try {
      const token = localStorage.getItem('token');
      await fetch('http://localhost:8080/api/whiteboards', {
        method: 'POST',
        headers: {
  ...
  const loadFromBackend = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8080/api/whiteboards/${subject}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      if (response.ok) {
        ...
        if (data.drawingActions) {
          setDrawingActions(data.drawingActions);
        }
      } else {
    ...
    <div>
      <div style={{ padding: '1rem' }}>
        ...
        <div style={{ display: 'flex', marginBottom: '1rem' }}>
          ...
          <button onClick={clearCanvas} style={{ backgroundColor: 'red', color: 'white', marginRight: '0.5rem' }}>
            Clear
          </button>
          <button onClick={saveToBackend} style={{ marginRight: '0.5rem' }}>
            Save
          </button>
